...
def _recalc_arrears_df(df, today):
    ...
    if hasattr(df[arr_col], 'dtype') and str(df[arr_col].dtype) == 'string':
        ...
        try:
            ...
            df.loc[mask, arr_col] = days.astype(str)
    return df
def build_split_workbook(account_data, amount_col, today=None, title_prefix="", templates=None, lang="en", per_account_dates=None, per_account_langs=None):
    if today is None:
        today = datetime.date.today()
    per_account_dates = per_account_dates or {}
    per_account_langs = per_account_langs or {}
    ...
    # Translate Document Type codes — use per-account language if available
    account_data = {
        acc: translate_doc_types(df, per_account_langs.get(str(acc), lang))
        for acc, df in account_data.items()
    }
    ...
    for idx, (acc, acc_df) in enumerate(account_data.items()):
        ...
        # Use per-account date and language if available
        ...
        r2 += 1
        _subtitle_removed = {
            "en": "Invoices not yet due removed",
            "nl": "Niet-vervallen facturen verwijderd",
            "fr": "Factures non échues supprimées",
        }.get(acc_lang, "Invoices not yet due removed")
        mr(ws, r2, 1, ncols,
           acc_today_str + "  \u00b7  " + f"{len(acc_df):,}" + f" lines  \u00b7  {_subtitle_removed}",
