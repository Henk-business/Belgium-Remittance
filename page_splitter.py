...
def show():
    ...
    # Pre-load any GitHub rules into session state on first load
    if not st.session_state.get("_rules_preloaded") and rules_gh_ok():
        try:
            from github_storage import list_github_templates as _list_tmpl
            # Load rules for any accounts that have config files
            # (We do a best-effort load; failures are silently ignored)
            import requests as _req
            import base64 as _b64, json as _json, streamlit as _st
            _hdrs = {"Authorization": f"Bearer {_st.secrets['github']['token']}",
                     "Accept": "application/vnd.github+json"}
            _repo = _st.secrets["github"]["repo"]
            _resp = _req.get(f"https://api.github.com/repos/{_repo}/contents/templates",
                             headers=_hdrs, timeout=10)
            if _resp.ok:
                for item in _resp.json():
                    if item["name"].startswith("config_") and item["name"].endswith(".json"):
                        acc_id = item["name"][7:-5]  # strip "config_" and ".json"
                        if not st.session_state.get(f"rule_{acc_id}"):
                            r2 = _req.get(item["download_url"], timeout=10)
                            if r2.ok:
                                rule_data = r2.json()
                                st.session_state[f"rule_{acc_id}"] = rule_data
            st.session_state["_rules_preloaded"] = True
        except Exception:
            st.session_state["_rules_preloaded"] = True  # don't retry
    ...
    for acc, acc_df_sel in account_data.items():
        ...
        # Rule lookup: session state is the primary source (always works)
        # GitHub is only used to pre-load rules at page start
        rule = st.session_state.get(f"rule_{acc}", None)

        # Try GitHub once if not in session
        if rule is None:
            try:
                if rules_gh_ok():
        ...
        if rule and rule.get("chunk_size", 0) > 0:
        ...
        elif rule and rule.get("columns"):
            try:
                acc_wb_bytes = build_chunked_sheet(acc_df_sel, str(acc), rule, today=ref_date)
                layout_label = "✓ custom columns"
            except Exception as e:
                acc_wb_bytes = build_split_workbook({acc: acc_df_sel}, amount_col, today=ref_date).getvalue()
                layout_label = f"ERROR: {e}"
        elif tmpl_bytes:
            try:
                ...
                acc_wb_bytes = build_split_workbook({acc: acc_df_sel}, amount_col, today=ref_date).getvalue()
                layout_label = f"template error: {e}"
        else:
