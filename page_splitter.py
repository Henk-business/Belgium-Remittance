...
def show():
    ...
    for acc_idx, (acc, acc_df_sel) in enumerate(account_data.items()):
        ...
        with st.expander(
            ...
            # Get rule for this account — load fresh each time (no cache issues)
            rule = None
            if rules_gh_ok():
                rule = _load_rule_direct(str(acc))
            if rule and (rule.get("chunk_size", 0) > 0 or rule.get("columns")):
                st.success(
                    f"\u2713 Custom rules active for account {acc}  \u00b7  "
                    f"chunk size \u20ac{rule.get('chunk_size',0):,.0f}  \u00b7  "
                    f"total position: {rule.get('total_position','bottom')}"
                )
            ...
            with sc1:
                if st.button(f"\U0001f4be  Save rules for {rule_acc}", key="rule_save", type="primary"):
                    ...
                    if ok:
                        ...
                        st.error(msg)
            with sc2:
                if existing and st.button(f"\U0001f5d1  Delete rules for {rule_acc}", key="rule_del"):
