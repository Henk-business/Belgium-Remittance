...
def show():
    ...
    for acc, acc_df_sel in account_data.items():
        ...
        # Rule: check session state first (set when saved), then GitHub
        rule = st.session_state.get(f"rule_{acc}", None)
        if rule is None and rules_gh_ok():
            try:
        ...
        if rule and (rule.get("chunk_size", 0) > 0 or rule.get("columns")):
            try:
                ...
                layout_label = f"standard (chunk error: {e})"
        elif tmpl_bytes:
            try:
        ...
        if rule_acc:
            ...
            with sc1:
                if st.button(f"\U0001f4be  Save rules for {rule_acc}", key="rule_save", type="primary"):
                    # Always save to session state immediately (works without GitHub)
                    st.session_state[f"rule_{rule_acc}"] = rule_obj
                    ok, msg = save_rule_github(rule_acc, rule_obj)
                    if ok:
                        invalidate_rule_cache()
                        st.success(msg + " (also saved to session)")
                    else:
                        # GitHub failed but session state is set - will work this session
                        st.warning(f"GitHub save failed: {msg}\nRule saved to session only — will apply this session but won\'t persist after refresh.")
                    st.rerun()
            with sc2:
