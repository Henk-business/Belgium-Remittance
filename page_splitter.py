...
def show():
    ...
    for acc_idx, (acc, acc_df_sel) in enumerate(account_data.items()):
        ...
        with st.expander(
            ...
            # Load rule directly from GitHub (bypasses cache)
            rule = None
            rule_error = None
            if rules_gh_ok():
                try:
            ...
            if rule_error:
                st.warning(f"Could not load rule from GitHub: {rule_error}")

            if rule and (rule.get("chunk_size", 0) > 0 or rule.get("columns")):
                try:
                    acc_wb_bytes = build_chunked_sheet(
                        acc_df_sel, str(acc), rule, today=ref_date
                    )
                    st.success(
                        f"\u2713 Custom rules applied  \u00b7  "
                        f"chunk \u20ac{rule.get('chunk_size',0):,.0f}  \u00b7  "
                        f"total: {rule.get('total_position','bottom')}"
                    )
                except Exception as e:
                    st.error(f"Rule error: {e} — using standard layout")
                    acc_wb_bytes = build_split_workbook(
                        {acc: acc_df_sel}, amount_col, today=ref_date,
                        title_prefix=f"Account {acc} \u2014 ",
                    ).getvalue()
            elif not rule and rules_gh_ok():
                # Rule not found — show message so user knows
                st.caption(
                    f"No custom rules saved for account {acc}. "
                    "Use the template section below to add rules."
                )
                if tmpl_bytes:
                    acc_wb_bytes = build_template_sheet(
                        str(acc), acc_df_sel, tmpl_bytes, amount_col, today=ref_date
                    )
                    st.caption("\u2713 Custom template applied")
                else:
                    acc_wb_bytes = build_split_workbook(
                        {acc: acc_df_sel}, amount_col, today=ref_date,
                        title_prefix=f"Account {acc} \u2014 ",
